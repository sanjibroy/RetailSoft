...
$query_vouchers =   "SELECT slno,subvchtype FROM tbl_vouchertype WHERE primaryvchtype='Receipt'";
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
   ...
   foreach ($vouchers as $option) {
    ...
    if($paymentMode!="Credit")
    {
        ...
        //$paymentMaster      = array($paymentId, $voucherDate, $voucherType, $partyId, $totalAmount, $currentStatus, $userId, $today, $paymentType, $isSettled);   
        ...
        {
            $error=1;
            $errorMsg    .=   "Error in receipt master";
        } 
        ...
        if($paymentMode=="Cheque"){
            ...
            //payment mop
            //$paymentMop         =   array($paymentId,$totalAmount,$voucherType,$chequeNo,$chequeDate,'','',$bank);
        ...
        if($paymentMode=="UPI"){
            ...
            $utr            =$_POST["utr"];
            //payment mop
            //$paymentMop         =   array($paymentId,$totalAmount,$voucherType,'','','',$utr,$bank);
        ...
        $currentStatus          =   0;
        //$transMasterValues      = array($paymentId,$voucherDate,$voucherType,$partyId,$totalAmount,"","",$currentStatus,"","","","",$userId,$today);
        ...
        {
            $error=1;
            $errorMsg    .=   "Error in receipt id update";
        }
    }
    ...
    if ($error==1)
    {
        $result="Failed";
        mysqli_rollback($dbh);
        $msg="Receipt Could Not Be Saved";
    }
    else
    {
        $result="Success";
